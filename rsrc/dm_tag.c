...
{
//	gi.dprintf(DEVELOPER_MSG_GAME, "%s killed %s\n", attacker->classname, targ->classname);
//	gi.dprintf(DEVELOPER_MSG_GAME, "%x killed %x\n", attacker, targ);

	if(tag_token && targ && (targ == tag_owner))
	{
//		gi.dprintf(DEVELOPER_MSG_GAME, "owner died/suicided. dropping\n");
		Tag_DropToken(targ, FindItem( "Tag Token" ));
		tag_owner = NULL;
		tag_count = 0;
	}
//	else
//		gi.dprintf(DEVELOPER_MSG_GAME, "unrelated slaying\n");
}
...
//	gi.dprintf(DEVELOPER_MSG_GAME, "%x killed %x\n", attacker, victim);
	if(tag_token && tag_owner)
	{
		...
		if((scoreChange > 0) && tag_owner == attacker)
		{
			scoreChange = 3;
			tag_count++;
//			gi.dprintf(DEVELOPER_MSG_GAME, "tag total: %d\n", tag_count);
			if(tag_count == 5)
			{
//				gi.dprintf(DEVELOPER_MSG_GAME, "going to quad\n");
		...
		{
//			gi.dprintf(DEVELOPER_MSG_GAME, "owner killed by another player.\n");
			scoreChange = 5;
...
//		else
//			gi.dprintf(DEVELOPER_MSG_GAME, "unaffected slaying\n");
	}
//	else
//		gi.dprintf(DEVELOPER_MSG_GAME, "no tag token?\n");
...
//	gi.dprintf(DEVELOPER_MSG_GAME, "tag token picked up by %x\n", other);
...
void Tag_Respawn (edict_t *ent)
{
	...
	if(spot == NULL)
	{
//		gi.dprintf(DEVELOPER_MSG_GAME, "No open spawn point, waiting...\n");
		ent->nextthink = level.time + 1;
		return;
	}

//	gi.dprintf(DEVELOPER_MSG_GAME, "Relocating\n");
...
void Tag_MakeTouchable (edict_t *ent)
{
	...
	{
//		gi.dprintf(DEVELOPER_MSG_GAME, "spawned in slime or lava. quick relocate\n");
		tag_token->nextthink = level.time + 3;
	}
	else
	{
//		gi.dprintf(DEVELOPER_MSG_GAME, "spawned in the clear. regular relocate\n");
...
//	if(ent->client)
//		gi.dprintf(DEVELOPER_MSG_GAME, "%s dropped the tag token\n", ent->client->pers.netname);
//	else
//		gi.dprintf(DEVELOPER_MSG_GAME, "non-client dropped the tag token  (%s)\n", ent->classname);
